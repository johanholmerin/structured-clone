...
function compare_primitive(actual, input) {
  expect(actual).equal(input);
}
function compare_Array(callback) {
  return function(actual, input) {
    expect(typeof actual).not.equal("string");
    expect(actual instanceof Array, "instanceof Array").to.be.true;
    expect(actual).not.equal(input);
    expect(actual.length).equal(input.length, "length");
...
function compare_Object(callback) {
  return function(actual, input) {
    expect(typeof actual).not.equal("string");
    expect(actual instanceof Object, "instanceof Object").to.be.true;
    expect(actual instanceof Array, "instanceof Array").to.be.false;
    expect(actual).not.equal(input);
    callback(actual, input);
  };
}
...
function compare_Boolean(actual, input) {
  expect(typeof actual).not.equal("string");
  expect(actual instanceof Boolean, "instanceof Boolean").to.be.true;
  expect(String(actual)).equal(String(input), "converted to primitive");
  expect(actual).not.equal(input);
}
...
function compare_obj(what) {
  ...
  return function(actual, input) {
    expect(typeof actual).not.equal("string");
    expect(actual instanceof Type, "instanceof " + what).to.be.true;
    expect(Type(actual)).equal(Type(input), "converted to primitive");
    expect(actual).not.equal(input);
  };
}
...
function compare_Date(actual, input) {
  expect(typeof actual).not.equal("string");
  expect(actual instanceof Date, "instanceof Date").to.be.true;
  expect(Number(actual)).equal(Number(input), "converted to primitive");
  expect(actual).not.equal(input);
}
...
function compare_RegExp(expected_source) {
  ...
  return function(actual, input) {
    expect(typeof actual).not.equal("string");
    expect(actual instanceof RegExp, "instanceof RegExp").to.be.true;
    expect(actual.global).equal(input.global, "global");
    expect(actual.ignoreCase).equal(input.ignoreCase, "ignoreCase");
    expect(actual.multiline).equal(input.multiline, "multiline");
    expect(actual.source).equal(expected_source, "source");
    expect(actual.sticky).equal(input.sticky, "sticky");
    expect(actual.unicode).equal(input.unicode, "unicode");
    expect(actual.lastIndex).equal(0, "lastIndex");
    expect(actual).not.equal(input);
  };
}
...
async function compare_Blob(actual, input, expect_File) {
  expect(typeof actual).not.equal("string");
  expect(actual instanceof Blob, "instanceof Blob").to.be.true;
  if (!expect_File)
    expect(actual instanceof File, "instanceof File").to.be.false;
  expect(actual.size).equal(input.size, "size");
  expect(actual.type).equal(input.type, "type");
  expect(actual).not.equal(input);
  const ab1 = await new Response(actual).arrayBuffer();
  const ab2 = await new Response(input).arrayBuffer();
  expect(ab1.byteLength).equal(ab2.byteLength, "byteLength");
  const ta1 = new Uint8Array(ab1);
  const ta2 = new Uint8Array(ab2);
  for (let i = 0; i < ta1.size; i++) {
    expect(ta1[i]).equal(ta2[i]);
  }
}
...
function compare_File(actual, input) {
  expect(actual instanceof File, "instanceof File").to.be.true;
  expect(actual.name).equal(input.name, "name");
  expect(actual.lastModified).equal(input.lastModified, "lastModified");
  compare_Blob(actual, input, true);
}
...
function compare_FileList(actual, input) {
  expect(typeof actual).not.equal("string");
  expect(actual instanceof FileList, "instanceof FileList").to.be.true;
  expect(actual.length).equal(input.length, "length");
  expect(actual).not.equal(input);
...
function compare_ArrayBufferView(view) {
  ...
  return function(actual, input) {
    expect(typeof actual).not.equal("string");
    expect(actual instanceof Type, "instanceof " + view).to.be.true;
    expect(actual.length).equal(input.length, "length");
    expect(actual.buffer).not.equal(input.buffer, "buffer");
    for (let i = 0; i < actual.length; ++i) {
      expect(actual[i]).equal(input[i], "actual[" + i + "]");
    }
  };
}
function compare_ImageData(actual, input) {
  expect(typeof actual).not.equal("string");
  expect(actual.width).equal(input.width, "width");
  expect(actual.height).equal(input.height, "height");
  expect(actual.data).not.equal(input.data, "data");
...
function check_circular_property(prop) {
  return function(actual) {
    expect(actual[prop]).equal(actual);
  };
}
check(
...
function check_identical_property_values(prop1, prop2) {
  return function(actual) {
    expect(actual[prop1]).equal(actual[prop2]);
  };
}
check(
...
function check_absent_property(prop) {
  return function(actual) {
    expect(prop in actual).to.be.false;
  };
}
check(
...
function check_writable_property(prop) {
  return function(actual, input) {
    expect(actual[prop]).equal(input[prop]);
    actual[prop] += " baz";
    expect(actual[prop]).equal(input[prop] + " baz");
  };
}
check(
...
//   expect(typeof actual).not.equal('string');
//   expect(actual instanceof ImageBitmap, 'instanceof ImageBitmap').to.be.true;
//   expect(actual).not.equal(input);
...
//     const copy = await structuredClone(bm);
...
check(
  ...
  (copy, original) => {
    ...
    expect(copy instanceof Object).to.be.true;

    const newProto = { some: "proto" };
    // Must not throw:
    Object.setPrototypeOf(copy, newProto);

    expect(Object.getPrototypeOf(copy), newProto);
  }
);

describe("new", function() {
  structuredCloneBatteryOfTests.forEach(test => {
    it(test.description, test.f);
  });
});
